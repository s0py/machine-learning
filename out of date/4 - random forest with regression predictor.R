...
random_forest_model <- randomForest(Defaulted ~ ., data = training_df, ntree = 1000)
...
ggplot(conf_matrix_df, aes(x = prediction, y = default)) +
  ...
  theme_minimal()
ggsave(filename="plots/4b - plot - random forest regression - confusion matrix - 1000 trees.png")
results$error <- xor(as.numeric(results$Defaulted), as.numeric(results$Prediction))
accuracy
1-mean(dt$Defaulted)
accuracy - (1-mean(dt$Defaulted))

# Saying no one will default is right 78% of the time
# This model is right 82% of the time
# This is a slightly better score without rounding than with categorical predictors